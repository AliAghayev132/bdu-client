...
export default function NewsModal({ isOpen, onClose, onSubmit, initialData = null, isLoading = false }) {
  ...
  const handleImageUpload = async (file) => {
    ...
    try {
      ...
      const response = await fetch('http://localhost:3001/api/admin/news/upload-image', {
        method: 'POST',
        body: uploadFormData,
        credentials: 'include',
        headers: {
    ...
    <div className="fixed inset-0 z-50 overflow-hidden">
      ...
      <div className="absolute inset-0 flex items-center justify-center p-4">
        <div className="relative bg-white w-full h-full max-w-7xl max-h-[95vh] rounded-xl shadow-2xl flex flex-col">
          {/* Header */}
          <div className="flex items-center justify-between px-6 py-4 border-b border-gray-200 bg-gradient-to-r from-blue-600 to-indigo-600 text-white rounded-t-xl">
            <div className="flex items-center gap-4">
              <h2 className="text-2xl font-bold">
                {initialData ? 'Xəbəri Redaktə Et' : 'Yeni Xəbər Yarat'}
              </h2>
              <div className="flex items-center gap-2 text-sm">
                <span className={`px-3 py-1 rounded-full ${currentStep === 1 ? 'bg-white text-blue-600' : 'bg-blue-500'}`}>
                  1. Məzmun
                </span>
                <span className="text-blue-200">→</span>
                <span className={`px-3 py-1 rounded-full ${currentStep === 2 ? 'bg-white text-blue-600' : 'bg-blue-500'}`}>
                  2. Parametrlər
                </span>
              </div>
            </div>
              ...
              <div className="space-y-6 max-w-4xl mx-auto">
                <div>
                  ...
                  <input
                    ...
                    className="block w-full text-sm text-gray-500 file:mr-4 file:py-3 file:px-6 file:rounded-lg file:border-0 file:text-sm file:font-semibold file:bg-blue-600 file:text-white hover:file:bg-blue-700 file:cursor-pointer"
                  />
                  {coverImagePreview && (
