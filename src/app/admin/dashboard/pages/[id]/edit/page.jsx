...
// API
import { useGetPageByIdQuery, useUpdatePageMutation } from '@store/api/pagesApi';
...
export default function EditPagePage() {
  ...
  const { id } = useParams();
  const { data: pageData, isLoading: isFetching } = useGetPageByIdQuery(id);
  const [updatePage, { isLoading: isUpdating }] = useUpdatePageMutation();
  const form = usePageForm(pageData);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      await updatePage({ id, formData: submitData }).unwrap();
    ...
    return (
      <div className="fixed inset-0 z-50 bg-white flex items-center justify-center">
        <div className="flex flex-col items-center gap-4">
          ...
          <p className="text-gray-500 font-medium">Səhifə yüklənir...</p>
        </div>
      </div>
    );
  }
