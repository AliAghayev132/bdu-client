...
import { useGetNewsByIdQuery, useUpdateNewsMutation } from '@store/api/newsApi';
...
export default function EditNewsPage() {
  ...
  const form = useNewsForm(newsData);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      await updateNews({ id, formData: submitData }).unwrap();
    ...
    return (
      <div className="fixed inset-0 z-50 bg-white flex items-center justify-center">
        <div className="flex flex-col items-center gap-4">
          ...
          <p className="text-gray-500 font-medium">Xəbər yüklənir...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="fixed inset-0 z-50 bg-white flex flex-col">
      <NewsFormHeader title="Xəbəri Redaktə Et" currentStep={form.currentStep} />
